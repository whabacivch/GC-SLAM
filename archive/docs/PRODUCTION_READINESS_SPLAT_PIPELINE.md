...
- **Frontend/sensors:** `visual_feature_extractor`, `lidar_camera_depth_fusion`, `splat_prep`, `lidar_surfels` (sensor I/O and evidence/feature extraction only).
- **Common:** `bev_pushforward`, `splat_batch` (packed batch with N_max, M_max, K_max, masks, neighbor indices), `ma_hex_web`.
- **Backend:** `rendering.py` (splat rendering: EWA, vMF shading, fBm — output from state/map, not sensor input).
...
- **No** entries for: visual_extractor, splat_prep, bev_pushforward, sinkhorn_ot, ot_fusion, rendering.
...
- **Runtime manifest:** `test_geometric_compositional_invariants` (or equivalent) should assert manifest contains expected backends and, when camera is enabled, camera topic and enabled_sensors. Today manifest is logged; tests can parse published JSON.
