...
- **Additive fusion**: `fuse_info()` implements $\xi_{fused} = \sum_i \xi_i$, $\Lambda_{fused} = \sum_i \Lambda_i$ (Section 5.4)
...
**What We Have**:
- `fisher_information(L)` returns $g = \Lambda$ (Fisher metric for Gaussian)
